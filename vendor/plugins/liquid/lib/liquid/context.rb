module Liquid
  ...
  class Context
    ...
    def local_strainer
      @local_strainer ||= Class.new(Strainer)
    end
    
    def filters
      @filters ||= local_strainer.new
    end
    ...
    def add_filters(filter)
      ...
      raise ArgumentError, 'Cannot add filters to local context after Context#filters was called or rendering started' unless @filters.nil?
      local_strainer.send(:include, filter)
    end
                              
    def invoke(method, *args)
      if local_strainer.ok?(method)
        filters.__send__(method, *args)
      else
        return args[0]
      end        
    end
