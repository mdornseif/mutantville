module Liquid
  ...
  class Block
      ...
      while token = tokens.shift 
        ...
        when /^#{BlockStartTag}/                   
          if token =~ /^#{BlockStartTag}\s*(\w+)\s*(.*)?#{BlockEndTag}$/
            ...
            raise SyntaxError, "Tag '#{token}' was not properly terminated with regexp: #{BlockEndTag.inspect} "
          end
    ...
    def create_variable(token)
      ...
      raise SyntaxError.new("Variable '#{token}' was not properly terminated with regexp: #{VariableEndTag.inspect} ")
    end
