module LoginHelper
  ...
  def form_input(helper_method, field_name, options = {}, form_name = nil)
    form_name = "login" if form_name.nil?
    ...
    when /^.*button$/
      ...
      prompt = l(:"#{lname}")
      if LoginSystem::CONFIG[:two_column_input]
<<-EOL
...
<<-EOL
  ...
  def head_helper(options = {})
    ...
    if not opts[:error].nil? and opts[:error]
     error = error_messages_for('login')
     if not error.nil?
       error = error + "<br/>"
       s = s + error
     end
...
EOL
  ...
  def changeable(login, field)
    if login.new_record? or LoginSystem::CONFIG[:changeable_fields].include?(field)
      :text_field
    else
      :read_only_field
    end
