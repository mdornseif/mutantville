...
# and ist is somewhat involved since it shadows the original user object
class Login < ActiveRecord::Base
  ...
  def set_delete_after
    hours = LoginSystem::CONFIG[:delayed_delete_days] * 24
  ...
  def token_lifetime(hours = nil)
    if hours.nil?
      LoginSystem::CONFIG[:security_token_life_hours] * 60 * 60
    else
      hours * 60 * 60
    end
  end
